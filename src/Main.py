...
from server_manager import recover_running_servers, auto_start_servers, monitor_servers
...
from web_ui import start_web_ui, is_web_ui_started


_monitor_task = None
_desktop_ui_started = False
_web_ui_started = False
...
@bot.event
async def on_ready():
    global _monitor_task, _desktop_ui_started, _web_ui_started
    try:
        try:
            ...
            logging.exception("[STEAMCMD] Setup fehlgeschlagen – fahre trotzdem fort")
        init_db()
        cleanup_old_logs(get_log_retention_days())
        load_server_paths()
        load_server_configs()
        # Recovery/Auto-Start kapseln, damit ein Fehler den restlichen
        # Bot-Start (UI, Status-Panel) nicht verhindert.
        try:
            await recover_running_servers()
        except Exception:
            logging.exception("[BOOT] recover_running_servers fehlgeschlagen")
        try:
            await auto_start_servers()
        except Exception:
            logging.exception("[BOOT] auto_start_servers fehlgeschlagen")
        ...
        if not _web_ui_started:
            async def _refresh_discord_from_web():
                channel = bot.get_channel(CHANNEL)
                if channel:
                    await refresh_status_panel(channel, user=None)

            started = start_web_ui(
                asyncio.get_running_loop(),
                refresh_callback=_refresh_discord_from_web,
            )
            _web_ui_started = bool(started)
...
async def _run_ui_only() -> None:
    global _desktop_ui_started, _web_ui_started
    ok_cmd, cmd_message = await asyncio.to_thread(ensure_steamcmd_available)
    if ok_cmd:
        logging.info("[STEAMCMD] %s", cmd_message)
    else:
    ...
    if not _desktop_ui_started:
        ...
        _desktop_ui_started = bool(started)
    if not _web_ui_started:
        started = start_web_ui(asyncio.get_running_loop(), refresh_callback=None)
        _web_ui_started = bool(started)

    if not _desktop_ui_started and not _web_ui_started:
        logging.error("[UI] Keine lokale UI aktiv. DGSM_DESKTOP_UI_ENABLED/DGSM_WEB_UI_ENABLED pruefen.")
        return

    # Avoid startup race: UI thread may need a moment until the first visible window exists.
    if _desktop_ui_started:
    ...
    while is_desktop_ui_started() or is_web_ui_started():
